...
def _print_trace_report(calls, arch):
    """Print discovered op call paths + write an auto-generated op-hook yaml.

    ``calls``: {op_name: [(caller_loc, [shapes...]), ...]}.
    Writes models/hooks/<arch>_trace.yaml with paired input+output op hooks,
    deduped (skips torch.ops._C_ascend.* underlying impls that DeviceOperator.*
    wraps, so the same call isn't double-captured). ids carry a namespace
    prefix (dq/tn) so same-named ops across namespaces don't collide.
    """
    ...
    lines = [
        ...
        f"# {len(calls)} ops discovered; {len(kept)} kept ({skipped} torch.ops._C_ascend.*",
        "# underlying impls skipped -- they are wrapped by DeviceOperator.* and would",
        "# double-capture). Each op has paired input+output hooks so in-vs-out cosine",
        "# isolates whether the op diverges on identical input. Edit call_index as needed.",
        f"architecture: {arch}",
    ...
    try:
        ...
        with open(out_path, "w") as f:
            f.write(yaml_text)
        print(f"  Wrote {out_path} ({len(kept)} ops, {len(kept)*2} hooks)")
...
def _run_trace(args, cfg):
    ...
    if model is None:
        ...
        _print_trace_report(calls, cfg.architecture)
    else:
        ...
        try:
            ...
            tracer.uninstall()
        _print_trace_report(dict(tracer._calls), cfg.architecture)
    print('[trace] done')
